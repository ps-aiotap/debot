...
# Skip database setup if PostgreSQL not configured
if not all(
    [os.getenv("POSTGRES_HOST"), os.getenv("POSTGRES_PORT"), os.getenv("POSTGRES_USER")]
):
    print("PostgreSQL not configured, skipping database setup")
    engine = None
    SessionLocal = None
    Base = None
    Document = None
    ...
    def create_database_if_not_exists():
        try:
            ...
            cursor.execute(
                f"SELECT 1 FROM pg_database WHERE datname='{os.getenv('POSTGRES_DB')}'"
            )
